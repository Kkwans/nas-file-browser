...
func printSettings(ser *settings.Server, set *settings.Settings, auther auth.Auther) error {
	...
	writef("Sign up:\t%t\n", set.Signup)
	writef("Hide Login Button:\t%t\n", set.HideLoginButton)
	writef("Create User Dir:\t%t\n", set.CreateUserDir)
	writef("Logout Page:\t%s\n", set.LogoutPage)
	writef("Minimum Password Length:\t%d\n", set.MinimumPasswordLength)
	writef("Auth Method:\t%s\n", set.AuthMethod)
	writef("Shell:\t%s\t\n", strings.Join(set.Shell, " "))

	writef("\nBranding:\n")
	writef("\tName:\t%s\n", set.Branding.Name)
	writef("\tFiles override:\t%s\n", set.Branding.Files)
	writef("\tDisable external links:\t%t\n", set.Branding.DisableExternal)
	writef("\tDisable used disk percentage graph:\t%t\n", set.Branding.DisableUsedPercentage)
	writef("\tColor:\t%s\n", set.Branding.Color)
	writef("\tTheme:\t%s\n", set.Branding.Theme)
	...
	writef("\tTLS Cert:\t%s\n", ser.TLSCert)
	writef("\tTLS Key:\t%s\n", ser.TLSKey)
	writef("\tToken Expiration Time:\t%s\n", ser.TokenExpirationTime)
	writef("\tExec Enabled:\t%t\n", ser.EnableExec)
	writef("\tThumbnails Enabled:\t%t\n", ser.EnableThumbnails)
	writef("\tResize Preview:\t%t\n", ser.ResizePreview)
	writef("\tType Detection by Header:\t%t\n", ser.TypeDetectionByHeader)

	writef("\nTUS:\n")
	writef("\tChunk size:\t%d\n", set.Tus.ChunkSize)
	writef("\tRetry count:\t%d\n", set.Tus.RetryCount)

	writef("\nDefaults:\n")
	writef("\tScope:\t%s\n", set.Defaults.Scope)
	writef("\tDateFormat:\t%t\n", set.Defaults.DateFormat)
	writef("\tHideDotfiles:\t%t\n", set.Defaults.HideDotfiles)
	writef("\tLocale:\t%s\n", set.Defaults.Locale)
	writef("\tView mode:\t%s\n", set.Defaults.ViewMode)
	writef("\tSingle Click:\t%t\n", set.Defaults.SingleClick)
	writef("\tRedirect after Copy/Move:\t%t\n", set.Defaults.RedirectAfterCopyMove)
	writef("\tFile Creation Mode:\t%O\n", set.FileMode)
	writef("\tDirectory Creation Mode:\t%O\n", set.DirMode)
	writef("\tCommands:\t%s\n", strings.Join(set.Defaults.Commands, " "))
	writef("\tAce editor syntax highlighting theme:\t%s\n", set.Defaults.AceEditorTheme)
