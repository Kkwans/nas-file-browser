...
- 验证：更新媒体加载契约后，聚焦 Vitest `20/20`、前端 lint、typecheck、production build、GitHub CI 和 Compose CI 全部成功。NAS 本机 ARM64 镜像 `nas-file-browser:2026.8.25-phase10-media-r154` 已部署，容器 `running/healthy`，`/health` 返回 `{"status":"OK"}`。
- NAS 本机 Playwright 真实复测：对未命中缓存的 `/tmp/nfb-acceptance-large-cold-e-r83.jpg`（10000×7000）等待 4.8s 后，页面状态 `ready`，最终图片为 `1080×756` 的 `/api/preview/big`，资源记录中没有 `/api/raw`；控制台错误 `0`。同轮 VP9/Opus MKV 为 `readyState=4`、`duration=8.008s`、`seekable=[0,8.008]`、无 HLS；H.264/AAC MKV 保持兼容入口、无自动 HLS，错误 `0`。
- 当前结论：本轮已消除一个可复现的浏览器高内存卡顿根因，但未宣称所有媒体格式或所有超大图片都已完成性能优化；H.264/HEVC 等当前 Chromium 不支持的编码仍需用户主动兼容播放或外部播放器。下一步继续检查视频首帧/兼容任务失败路径、文件列表视觉和 Markdown 编辑体验。

### 2026-08-25 r155 任务与历史列表信息层级重排

- 真实 r154 页面审计确认：任务行把相对时间、用户、任务类型和重试标记全部挤在标题下方，右侧“查看结果/归档”又脱离同一基线；长列表中时间和动作像漂浮在边缘，文字与操作主次不清。
...
### 2026-08-25 r158 任务列表列基线与操作栏重排

- 用户继续指出任务中心不是单个顶部卡片的问题，而是整页文字、图标、排版以及列表中的时间和结果操作都缺少稳定的视觉基线。真实 r157 页面确认任务行内容虽然已经移除装饰性彩色左轨，但右栏没有列标题，时间与动作仍像悬浮在边缘，完成任务的主操作与普通归档操作也缺少层级差异。
- 修复：在任务列表中加入明确的“任务 / 最近更新与操作”列标题，桌面任务行固定为状态图标、内容、右侧时间与操作三列；右侧栏增加轻量中性分隔线，避免与内容混淆但不使用用户反感的彩色强调条。结果动作使用轻量蓝色语义背景和固定最小宽度，归档等次级动作继续保持中性；移动端隐藏桌面列标题、移除垂直分隔线，保留底部分隔操作行和 `44px` 触摸目标。任务数据、筛选、轮询、重试、取消、归档、结果弹窗和返回定位均未改变。
- TDD/静态验证：新增契约先在旧实现上得到 `1 failed / 9 passed`，修复后活动页契约 `10/10`、前端完整 Vitest `95 files / 371 tests`、ESLint、typecheck、production build 和 `git diff --check` 均通过。源码提交 `89a7b18d` 已推送，源码 CI `32842098175`、Docs `32842098218` 均成功。
...
### 2026-08-25 r160 活动页整体导航与时间操作栏精修

- 用户复核明确指出任务中心/操作历史的问题不是顶部单一区域，而是整页文字、图标、排版和列表项的视觉层级都不够精致，尤其时间与查看结果操作像漂浮在列表边缘。r159 的截图结构已经具备列基线，但仍保留偏重的胶囊式任务/历史切换、右栏对齐方向不一致以及结果按钮与普通归档动作层级不足。
- 修复：活动页切换器改为轻量下划线导航，移除外层胶囊背景；任务列表右侧扩为稳定的 `176px` 时间/操作栏，表头改为“更新时间 / 操作”，时间与按钮左对齐并使用同一列分隔；结果按钮新增明确的主操作样式，归档/恢复等动作保持次级语义；历史列表时间列扩为 `144px`，表头和记录时间统一左对齐，保留连续时间线；任务/历史列表容器使用轻量阴影和统一圆角，移动端仍隐藏桌面列头并保留独立操作行与触摸尺寸。
- TDD/静态验证：新活动页契约先在旧实现上得到 `1 failed / 11 passed`，修复后活动页契约 `12/12`；前端完整 Vitest `95 files / 373 tests`、ESLint、typecheck、production build、`git diff --check` 均通过。源码提交 `3bd91ec3` 已推送，Compose 发布提交 `e0e2a40e` 已推送；GitHub CI `32845764136`（前后端 lint、前端测试、Go race、构建、Docker）成功，Docs `32845764143` 成功。
- 发布：本机 ARM64 镜像 `nas-file-browser:2026.8.25-phase10-activity-r160`，镜像 ID `sha256:8a8448600c61caf5b3add8bf808b578834f42947bd174935d5a7d153c8dca774`；仅重建 `filebrowser`，r159 镜像与 Compose 保留作回滚。容器 `running/healthy`，`/health` 返回 `{"status":"OK"}`。未修改数据库、用户文件或未跟踪文件。
- 真实浏览器 Gate：当前 NAS 工作目录没有可解析的本地 Playwright 包；既有审计脚本引用 `C:/Users/Kkwans/...` Windows 路径，直接执行得到 `MODULE_NOT_FOUND`。因此本轮不能宣称 NAS 本地 Playwright 视觉验收通过，也未改动用户数据来伪造测试条件；r160 的视觉坐标、任务结果弹窗和移动端截图需要在可用的真实浏览器环境中补验。
- 当前结论：r160 完成活动页整体视觉语言和时间/操作栏的代码级精修并已部署，但不等于任务中心、操作历史或全站 UI 验收完成。下一步继续处理文件列表紧凑布局、文件夹/风险图标与媒体/Markdown 页面；任何浏览器复验发现的问题仍按独立模块小步提交、推送、部署。

### 2026-08-25 r161 文件列表紧凑布局可读性

- 用户继续指出全站文字、图标和排版不够精致；文件列表的紧凑布局中，文件名、元信息、修改时间和图标列的字阶与基线过弱，长列表阅读负担明显。
- 修复：保留四种布局、拖拽移动、风险语义、缩略图和选择行为；紧凑网格文件名提升至 `14px`，紧凑列表统一使用 `34px` 资源图标列、`60px` 最小行高、`14px` 文件名、`12px` 元信息和数字化时间列，减少图标与文字的上下漂移。未修改风险分类、路径、用户数据或未跟踪文件。
- 验证：新增文件列表排版契约先在旧实现上得到 `1 failed / 0 passed`，修复后聚焦测试 `1/1`；前端完整 lint、typecheck、Vitest `96 files / 374 tests` 和 `git diff --check` 均通过。源码提交 `60dc6c7f` 已推送。
- 发布：Compose 提交 `3a42222c` 已推送；NAS 本机 ARM64 镜像 `nas-file-browser:2026.8.25-phase10-listing-r161`，镜像 ID `sha256:e29d8a63f15d6ded251aa09cb6a4a2f48e5d99537e28437ec3c3d1efedaa9f30`；仅重建 `filebrowser`，r160 镜像与 Compose 保留作回滚。容器 `running/healthy`，`/health` 返回 `{"status":"OK"}`。
- 真实浏览器 Gate：当前 NAS 工作目录仍没有可解析的本地 Playwright 包，不能宣称紧凑列表坐标和截图已通过真实浏览器验收；需在可用的 NAS 浏览器环境补验四种布局、窄容器和触屏。

### 2026-08-25 r162 任务列表时间与操作侧栏再收敛

- 用户明确指出任务页面不是顶部单卡问题，而是整页文字、图标、排版以及列表项的时间和查看结果区域都缺少产品级精致度。r160 仍让桌面任务行的时间和动作在右侧栏中上下分散，视觉上像漂浮元素。
- 修复：任务表头和正文右侧轨道统一扩为 `236px`；更新时间与主要操作收敛到同一侧栏网格，时间值右对齐，查看结果、重试、取消、归档和恢复动作固定在同一操作轨道；内容区、状态图标、任务标题和元信息不变。移动端继续使用底部分隔操作行和 `44px` 触摸目标，不改变任务筛选、轮询、批量处理或结果弹窗。
- TDD/静态验证：新增契约先在旧实现上得到 `3 failed / 10 passed`，修复后活动页契约 `13/13`；前端完整 lint、typecheck、Vitest `96 files / 375 tests` 通过。
