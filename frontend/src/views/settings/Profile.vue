...
loadAccount();
watch(() => authStore.user?.id, loadAccount, { flush: "sync" });
watch(
  [controlsTimeoutSec, playbackMode, playbackRate, resumeMode, resumeMinSec],
  () => {
    if (!initializing && authStore.user?.id) void persistPlayerPrefsNow();
  },
  { flush: "sync" }
);
watch(
  ...
  () => {
    if (!initializing && authStore.user?.id) void persistAccountPrefsNow();
  },
  { flush: "sync" }
);
...
async function persistPlayerPrefsNow(): Promise<boolean> {
  ...
  try {
    await accountPreferences.save(data);
    return true;
  } catch (err) {
    if (err instanceof Error) $showError(err);
    return false;
